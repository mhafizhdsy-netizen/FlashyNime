...
import { Play, Calendar, Clock, Star, Bookmark, Share2, Check, Copy, X, Facebook, Twitter, RefreshCw, Trash2, ArrowLeft, ChevronDown, ChevronUp } from 'lucide-react';
import { getAnimeDetail, getDonghuaDetail, normalizeAnime } from '../services/api';
...
export const Detail = () => {
  ...
  const isDonghua = window.location.hash.includes('donghua');
  ...
  const fetchDetail = async () => {
    ...
    setError(false);
    try {
      const fetcher = isDonghua ? getDonghuaDetail : getAnimeDetail;
  ...
  const renderSynopsis = () => {
    if (Array.isArray(anime?.synopsis)) {
      return anime.synopsis.map((p, i) => <p key={i} className="mb-2">{p}</p>);
    }
    return <p>{anime?.synopsis}</p>;
  };
  ...
  if (error || !anime) return (
    <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center p-6 text-center">
       ...
       <div className="flex gap-4">
         <Link to="/"><Button variant="secondary">{t.watch.goBack}</Button></Link>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="flex-shrink-0 w-full max-w-[320px] mx-auto lg:mx-0 space-y-6 animate-fade-in-up">
             <div className="relative group rounded-2xl overflow-hidden shadow-2xl shadow-violet-900/30 border border-white/10 bg-slate-800">
               ...
               {/* Saved Indicator Overlay (Synced with Logic) */}
               {inWatchlist && (
             ...
             <div className="space-y-8">
                ...
                <div>
                   ...
                   <div className="flex flex-wrap gap-2">
                      {anime.genres?.map((g) => (
                         <Link key={g} to={`/browse?genre=${g.toLowerCase()}`}>
